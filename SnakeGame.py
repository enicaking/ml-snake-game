...
# THE NEW INTELLIGENT METHOD

def isBlockedWall(game):
    body_blocked = False
    # Bump itself
...
def predictPath(game):
    predictedpath = []
    snake_head = game.snake_body[0]
    while not (snake_head[0] == game.food_pos[0] and snake_head[1] == game.food_pos[1]):
        new_direction = choose_snake_direction(game, snake_head)
        next_step = move_snake_head(new_direction, snake_head)
        predictedpath.append(next_step)
    return predictedpath


def choose_snake_direction(game, snake_head):
    change_to = game.direction
    if not isBlockedWall(game):
        if game.food_pos[0] == snake_head[0]:
            # food and snake are at the same x coordinate
            if game.food_pos[1] < snake_head[1] and game.direction != 'DOWN':
                change_to = 'UP'
            elif game.food_pos[1] > snake_head[1] and game.direction != 'UP':
                change_to = 'DOWN'
            else:
                change_to = loop(game, False)

        else:
            if game.food_pos[0] < snake_head[0] and game.direction != 'RIGHT':
                change_to = 'LEFT'
            elif game.food_pos[0] > snake_head[0] and game.direction != 'LEFT':
                change_to = 'RIGHT'
            else:
...
def move_snake_head(direction, snake_head):
    if direction == 'UP':
        snake_head[1] -= 10
    elif direction == 'DOWN':
        snake_head[1] += 10
...
def print_state(game):
    ...
    print("Blocked?", isBlockedWall(game))

    print("Distance to food:", game.dist_x, game.dist_y)
    print("Predicted Path:", game.predictedpath)
...
def print_line_data(game):
    line_data = [FRAME_SIZE_X, FRAME_SIZE_Y, game.direction,
                 ...
                 isBlockedWall(game), game.dist_x, game.dist_y]
...
while True:
    ...
    if not game.food_spawn:
        ...
        game.predictedpath = predictPath(game)
